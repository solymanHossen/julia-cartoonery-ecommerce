...
$package                  = isset( $package ) && is_array( $package ) ? $package : array();
$shipping_methods         = isset( $shipping_methods ) && is_array( $shipping_methods ) ? $shipping_methods : array();
$index                    = isset( $index ) ? $index : 0;
$chosen_method            = isset( $chosen_method ) ? $chosen_method : '';
$package_name             = isset( $package_name ) ? $package_name : __( 'Shipping', 'woocommerce' );
$formatted_destination    = isset( $formatted_destination ) ? $formatted_destination : ( ! empty( $package['destination'] ) ? WC()->countries->get_formatted_address( $package['destination'], ', ' ) : '' );
...
<div class="woocommerce-shipping-totals shipping space-y-6">
	...
	<?php else : ?>
		<div class="rounded-2xl border border-dashed border-slate-200 dark:border-slate-700 p-4 bg-slate-50/80 dark:bg-slate-700/20 text-sm text-slate-500 dark:text-slate-400">
			<?php esc_html_e( 'Shipping will be calculated once your address is entered.', 'woocommerce' ); ?>
		</div>
	<?php endif; ?>
