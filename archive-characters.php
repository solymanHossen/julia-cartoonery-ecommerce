...
<main class="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-pink-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 py-16 lg:py-24 relative overflow-hidden">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 max-w-6xl mx-auto">
            <?php if ( have_posts() ) : while ( have_posts() ) : the_post(); ?>
                ...
                <div class="group relative flex flex-col items-center p-8 bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl rounded-[40px] shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.2)] border border-white/50 dark:border-slate-700/50 hover:-translate-y-2 hover:shadow-[0_20px_40px_rgb(0,0,0,0.08)] dark:hover:shadow-[0_20px_40px_rgb(0,0,0,0.3)] transition-all duration-300 ease-out">
                    ...
                    <div class="absolute top-8 w-48 h-48 bg-gradient-to-tr from-blue-200 to-pink-200 dark:from-blue-600/30 dark:to-pink-600/30 rounded-full blur-2xl opacity-50 group-hover:scale-110 transition-transform duration-500"></div>

                    <!-- Character Image -->
                    <div class="relative w-48 h-48 rounded-full overflow-hidden border-[6px] border-white dark:border-slate-700 shadow-xl mb-8 bg-slate-50 dark:bg-slate-800 z-10 group-hover:rotate-3 transition-transform duration-300">
                        <?php if(has_post_thumbnail()): ?>
                            <img src="<?php the_post_thumbnail_url('medium'); ?>" alt="<?php the_title_attribute(); ?>" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
                        <?php else: ?>
                    ...
                    <h3 class="font-['Bubblegum_Sans'] text-3xl text-slate-800 dark:text-white mb-2 z-10 tracking-wide"><?php the_title(); ?></h3>
                    <div class="w-12 h-1 bg-pink-400 rounded-full mb-6 z-10 opacity-50 group-hover:w-20 transition-all duration-300"></div>
                    ...
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="group-hover/btn:-translate-y-1 transition-transform duration-300"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" x2="12" y1="15" y2="3"/></svg>
                        <span>Download Free</span>
                    </button>
                </div>

            <?php endwhile; else: ?>
                <div class="col-span-full py-20 text-center bg-white/50 dark:bg-slate-800/50 backdrop-blur-sm rounded-[40px] border border-slate-200 dark:border-slate-700">
                    <svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="mx-auto mb-4 text-slate-400"><circle cx="12" cy="12" r="10"/><path d="m15 9-6 6"/><path d="m9 9 6 6"/></svg>
                    <h3 class="text-2xl font-['Bubblegum_Sans'] text-slate-700 dark:text-slate-300 mb-2">No characters yet!</h3>
        ...
        <div id="modalContent" class="relative bg-white dark:bg-slate-800 p-8 sm:p-10 rounded-[2.5rem] shadow-2xl max-w-md w-full transform scale-90 translate-y-8 transition-all duration-500 ease-out border border-white/20">
            ...
            <div class="text-center relative z-0">
                ...
                <div class="relative w-24 h-24 mx-auto mb-8">
                    ...
                    <div class="absolute inset-2 bg-red-50 dark:bg-red-800/50 rounded-full flex items-center justify-center border border-red-100 dark:border-red-700/50 shadow-inner">
                        <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-red-500 dark:text-red-400 translate-x-0.5"><path d="M2.5 17a24.12 24.12 0 0 1 0-10 2 2 0 0 1 1.4-1.4 49.56 49.56 0 0 1 16.2 0A2 2 0 0 1 21.5 7a24.12 24.12 0 0 1 0 10 2 2 0 0 1-1.4 1.4 49.55 49.55 0 0 1-16.2 0A2 2 0 0 1 2.5 17"/><path d="m10 15 5-3-5-3z"/></svg>
                    </div>
                </div>
                ...
                <div id="modalSuccessArea" class="hidden overflow-hidden">
                    <div class="bg-emerald-50 dark:bg-emerald-900/30 border border-emerald-100 dark:border-emerald-800 text-emerald-600 dark:text-emerald-400 p-6 rounded-2xl flex flex-col items-center justify-center gap-3 font-bold animate-in zoom-in duration-300 shadow-inner">
                        <div class="w-12 h-12 bg-emerald-100 dark:bg-emerald-800/50 rounded-full flex items-center justify-center mb-1">
                            <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" class="text-emerald-500 animate-[bounce_1s_ease-in-out_infinite]"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><path d="m9 11 3 3L22 4"/></svg>
                        </div>
    ...
    <!-- TOAST NOTIFICATION -->
    <div id="toastNotification" class="fixed bottom-8 right-8 bg-slate-800 dark:bg-white text-white dark:text-slate-900 px-6 py-4 rounded-2xl shadow-2xl font-bold flex items-center gap-4 transform translate-y-24 opacity-0 transition-all duration-500 z-50 border border-slate-700 dark:border-white/20">
        <div class="w-8 h-8 bg-emerald-500 rounded-full flex items-center justify-center shrink-0">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><path d="m9 11 3 3L22 4"/></svg>
        </div>
