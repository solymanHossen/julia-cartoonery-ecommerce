<?php
/**
 * Julia's Cartoonery Theme Functions
 */

if (!defined('ABSPATH')) {
	exit;
}

require_once get_theme_file_path('inc/theme-setup.php');
require_once get_theme_file_path('inc/woocommerce-dependency-check.php');
require_once get_theme_file_path('inc/enqueue-scripts.php');
require_once get_theme_file_path('inc/post-types.php');
require_once get_theme_file_path('inc/menu-setup.php');
require_once get_theme_file_path('inc/woocommerce-functions.php');
require_once get_theme_file_path('inc/admin-wishlist.php');
require_once get_theme_file_path('inc/carousel-slider.php');
require_once get_theme_file_path('inc/videos-section.php');
require_once get_theme_file_path('inc/search-api.php');
